...
exports.handler = async (event) => {
  ...
  let ValidateShowInput = (showName, venueName, showDate, showTime) => {
        return new Promise((resolve, reject) => {
          ...
          date+= "/" + year;
            pool.query("SELECT * FROM shows WHERE showName=? AND venueID=? AND showDate=? AND showTime=?", [showName, idOfVenue, date, time], (error, rows) => {
                if (error) { return reject(error); }
                console.log(rows)
  ...
  let checkIfNotActive = (showName, venueName, showDate, showTime) => {
      return new Promise((resolve, reject) => {
        let [hours, mins] = parseTime(event.showTime);
          let time = hours + ":";
          if(mins < 10) {
  ...
  let DeleteShow = (showName, venueName, showDate, showTime) => {
      return new Promise((resolve, reject) => {
        let [hours, mins] = parseTime(event.showTime);
          let time = hours + ":";
          if(mins < 10) {
          ...
          date+= "/" + year;
          pool.query("DELETE FROM shows WHERE showName=? AND venueID=? AND showDate=? AND showTime=?", [showName, idOfVenue, date, time], (error, rows) => {
              if (error) { return reject(error); }
              console.log(rows);
    ...
    try{
      ...
      if(isVenueIDFound === 0){ // if venue doesn't exist
          ...
          if(!isVenueValid){ //if we are NOT authorised
              throw "Not authorized to delete a show for this venue.";
          }
          else{ // if we are authorised
              const can_delete = await ValidateShowInput(event.showName, event.venueName, event.showDate, event.showTime);
              if(!can_delete){
                   throw "A show in this venue at this time doesn't exist.";
              }
              else{
                ...
                const isShowActive = await checkIfNotActive(event.showName, event.venueName, event.showDate, event.showTime);
                
                if(!isShowActive){
                  throw "Cannot delete show that is active."
                }
                else{
                  let show = await DeleteShow(event.showName, event.venueName, event.showDate, event.showTime);
